	...
	var position = function () {
		...
		this.init = function () {
			...
			jQuery( '.wpb-rs-value select' ).each( function () {
				var make_select2 = jQuery( this ).data( 'make_select2' );
		...
		this.update_rule_select_names = function () {

			/* Make sure all select2 elements have the right value */
			jQuery( 'input.wpb-rs-select' ).each( function () {
		...
		this.load_value_select = function ( $triggered_obj ) {
			...
			jQuery.ajax( {
				...
				var make_select2 = select.data( 'make_select2' );
