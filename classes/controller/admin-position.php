...
final class Admin_Position_Controller {
	 ...
	 * @since 2.0.0
	 *
	 * @param null|\wpbuddy\rich_snippets\Position_Rule $rule
	 */
	...
	public function print_value_select( $rule = null ) {
		...
		switch ( $rule_param ) {
			...
			case 'page_parent':
				...
				if ( ! empty( $rule_value ) ) {
					$values[ $rule_value ] = sprintf(
						'%s (%s, %d)',
						get_the_title( $rule_value ),
						get_post_type( $rule_value ),
						$rule_value
					);
				}
		 ...
		 * @since 2.0.0
		 *
		 * @param array                                     $var  The current values.
		 * @param null|\wpbuddy\rich_snippets\Position_Rule $rule The current rule.
		 *
		 */
